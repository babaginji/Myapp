...
# Blueprint 名を ai_diagnosis_bp に統一

ai_diagnosis_bp = Blueprint(
    "ai_diagnosis",
    __name__,
    template_folder="templates",
    static_folder="static",  # ここ重要
    static_url_path="/ai_diagnosis/static",
)


@ai_diagnosis_bp.route("/", methods=["GET", "POST"])
def ai_home():
...
def result_main():
    ...
    # メイン判定に対応する説明
    main_desc1 = descriptions.get(main_result, "")
    main_desc2 = descriptions2.get(main_result, "")

    # サブ判定に対応する説明
    sub_desc1 = descriptions.get(sub_result, "")
    sub_desc2 = descriptions2.get(sub_result, "")

    return render_template(
        "ai_diagnosis/result_main.html",
        main_result=main_result,
        sub_result=sub_result,
        main_desc1=main_desc1,
...
@ai_diagnosis_bp.route("/result_books")
def result_books():
    # 必要な処理
    main_result = session.get("main_result")
    sub_result = session.get("sub_result")
    if not main_result:  # セッションがない場合は診断ページへ戻す
        return redirect(url_for("ai_diagnosis.ai_home"))

    return render_template(
        "ai_diagnosis/result_books.html", main_result=main_result, sub_result=sub_result
    )


@ai_diagnosis_bp.route("/result_learning")
def result_learning():
    # 必要な処理
    main_result = session.get("main_result")
    sub_result = session.get("sub_result")
    if not main_result:  # セッションがない場合は診断ページへ戻す
        return redirect(url_for("ai_diagnosis.ai_home"))
    return render_template(
        "ai_diagnosis/result_learning.html",
        main_result=main_result,
        sub_result=sub_result,
    )


@ai_diagnosis_bp.route("/result_start")
def result_start():
    # 必要な処理
    main_result = session.get("main_result")
    sub_result = session.get("sub_result")
    if not main_result:  # セッションがない場合は診断ページへ戻す
        return redirect(url_for("ai_diagnosis.ai_home"))
    return render_template(
        "ai_diagnosis/result_start.html", main_result=main_result, sub_result=sub_result
    )
